...
{
    public int numStimuli = 1;
    public enum stimType // your custom enumeration
    {
    face, letter, color, gabor
    };

    public stimType currStimType = stimType.face;
    public bool cueOn = true;
    private bool isOvert = true;
    public bool skipEyeTrackingCalibration = true;
    // Calculated for 0.5m distance from camera to deg0
    public double[] doublePos = {0.0}; // Different random horizontal positions
    // Names of different stimuli (Unity object names)
    private string[] faceStimuli = { "face1", "face2", "face3" };
    private string[] letterStimuli = { "CanvasE", "CanvasB", "CanvasP" };
    private string[] colorStimuli = { "redCircle", "greenCircle", "yellowCircle" };
    private string[] gaborStimuli = { "Gabor0", "Gabor90", "Gabor45" };
...
Press Spacebar when ready.",
        // Instruction 2
        @"This is stimuli 1. Press v to continue.",
        // Instruction 3
        @"This is stimuli 2. Press b to continue.",
        // Instruction 4
        @"This is stimuli 3. Press n to continue.",
        // Instruction 5
        @"Here are some practice rounds to familiarize you with the protocol.
Press Spacebar to begin.",
    };
    private string[] instrTextValues;
        ...
        if (Input.GetKeyDown(KeyCode.Return) || Input.GetKeyDown(KeyCode.KeypadEnter))
        {
            ...
            switch (currStimType)
            {
                case stimType.face:
                    currStimString = "Face";
                    break;
                case stimType.letter:
                    currStimString = "Letter";
                    break;
                case stimType.color:
                    currStimString = "Color";
                    break;
                case stimType.gabor:
                    currStimString = "Gabor";
                    break;
                default:
            ...
            logFile = dataPath + participantID + "-" + ovcoString + currStimString + "rtData-" + System.DateTime.Now.ToString("yyyy-MM-dd-HH-mm-ss") + ".csv";
    ...
    void Start()
    {
        ...
        switch (currStimType)
        {
            case stimType.face:
                ...
                break;
            case stimType.letter:
                stimuli = letterStimuli;
                break;
            case stimType.color:
                stimuli = colorStimuli;
                break;
            case stimType.gabor:
                stimuli = gaborStimuli;
                break;
            default:
                throw new ArgumentOutOfRangeException();
        }
        if (numStimuli < 3)
        {
            string[] newStimuli = new string[stimuli.Length - (3 - numStimuli)];
            Array.Copy(stimuli, newStimuli, newStimuli.Length);
            stimuli = newStimuli;
        }
        if(skipEyeTrackingCalibration){
            // Removes text
            instrText.GetComponent<TextMeshPro>().text = "";
            instrText.transform.position = GameObject.Find("disappearPos").transform.position;
            // Sets up for phase 2
            ...
            var filteredInstructions = new List<string>();

            for (int i = 0; i < instructions.Length; i++)
            {
                if ((numStimuli == 1 && (i == 2 || i == 3)) || (numStimuli == 2 && i == 3))
                {
                    continue;
                }
                filteredInstructions.Add(instructions[i]);
            }

            instrTextValues = filteredInstructions.ToArray();
    }
