...
{
    public enum protocolType // your custom enumeration
    {
    faceResize, faceNoResize, wordResize, wordNoResize, faceScaling, wordScaling
    };

    public protocolType currentProtocol = protocolType.faceResize;
    private bool cueOn = true;
    private bool isOvert = true;
    // stringPos is set to just have "1m" if it's the scaling protocols
    public string[] stringPos = {"0.5m","1m","2m","4m","8m"}; // Different random positions available (Unity object names)
    // Names of different stimuli (Unity object names)
    public int[] scales = {1, 2, 4, 8, 16}; // in degrees height, for scaling protocols
    private string[] faceStimuli = { "face1", "face2", "face3" };
    private string[] wordStimuli = { "word1", "word2", "word3" };
    ...
    public string[] instrTextValues = {
        ...
        // Instruction 3
        @"This is the stimulus 2. Press b to continue.",
        // Instruction 4
    ...
    private int calibrationPeriodCycle = 0;
    ...
    private float currentScale = 8;
    private int posIndex, stimIndex, scaleIndex; // Indices for position and stimuli respectively randomized later in code (need global scope since they're used in multiple functions)
    ...
    IEnumerator change()
    {
        ...
        // changes IV based on protocol
        if (currentProtocol == protocolType.faceResize || currentProtocol == protocolType.wordResize){
            float scaleValue = 0.5f * (float)Math.Pow(2, posIndex); // The desired scale for each axis
            Vector3 newScale = new Vector3(scaleValue, scaleValue, scaleValue); // Create a new Vector3 with the scale value for x, y, and z
            GameObject.Find(stimuli[stimIndex]).transform.localScale = newScale; // Set the new scale
        }
        else if (currentProtocol == protocolType.faceScaling || currentProtocol == protocolType.wordScaling){
            scaleIndex = rnd.Next(0, scales.Length);
            currentScale = scales[scaleIndex];
            float scaleValue = 0.125f * (float)Math.Pow(2, scaleIndex); // The desired scale for each axis
            Vector3 newScale = new Vector3(scaleValue, scaleValue, scaleValue); // Create a new Vector3 with the scale value for x, y, and z
            GameObject.Find(stimuli[stimIndex]).transform.localScale = newScale; // Set the new scale 
        }
        else if (currentProtocol == protocolType.faceNoResize || currentProtocol == protocolType.wordNoResize){
            currentScale = scales[4 - posIndex];
        }
        else{
            Debug.LogError("protocol not set correctly");
        }

        // Shows stimulus
        GameObject.Find(stimuli[stimIndex]).transform.position = GameObject.Find(stringPos[posIndex]).transform.position; // Changes to random position
    ...
    void phase2() // Participant name/ID input phase
    {
        ...
        if (Input.GetKeyDown(KeyCode.Return) || Input.GetKeyDown(KeyCode.KeypadEnter))
        {
            ...
            {
                case protocolType.faceResize:
                    protocolName = "faceResize";
                    break;
                case protocolType.faceNoResize:
                    protocolName = "faceNoResize";
                    break;
                case protocolType.wordResize:
                    protocolName = "wordResize";
                    break;
                case protocolType.wordNoResize:
                    protocolName = "wordNoResize";
                    break;
                case protocolType.faceScaling:
                    protocolName = "faceScaling";
                    break;
                case protocolType.wordScaling:
                    protocolName = "wordScaling";
                    break;
                default:
                    Debug.LogError("protocol not recognized");
                    break;
            }
            ...
            File.WriteAllText(logFile, "CueShowTime,ObjShowTime,ReactionTime,Location,Scaling,StimType,Guess,Correct\n");
    ...
    void phase3() // Start and instruction phase
    {
        if (Input.GetKeyDown(KeyCode.Space))
        {
            if (instrNum >= 0 && instrNum < stimuli.Length)
            {
                ...
                instrNum++;
                instrText.GetComponent<TextMeshPro>().text = instrTextValues[instrNum];
                GameObject.Find(stimuli[instrNum - 1]).transform.position = GameObject.Find("1m").transform.position;
            }
        ...
        if (!in_use)
        {
            ...
            if (responseKey != "")
            {
                ...
                if (start)
                {
                    ...
                    // Shows stimulus
                    log += stringPos[posIndex] + "," + currentScale + "," + stimuli[stimIndex] + "," + responseKey + ","; // independentVar, StimType, Guess
                    if (stimuli[stimIndex] == responseKey)
                    {
                        log += "True\n";
                    }
